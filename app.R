...
server <- function(input, output, session) {
  ...
  filter_options <- reactive({
    tryCatch({
      ...
      df<-df
      list(
        date_range_selectivity = dateRangeInput("date_range_selectivity", "Sélectionnez une plage de dates", start = "2024-01-01", end = "2024-12-31"),
  ...
  filtered_data_selectivity <- reactive({
    req(input$date_range_selectivity) # Ensure date range input is set
    ...
    if (!is.null(input$bureau_filter_selectivity) && length(input$bureau_filter_selectivity) > 0) {
      df_filtered <- df_filtered[df_filtered$BUREAU %in% input$bureau_filter_selectivity, ]
    }
    if (!is.null(input$modele_filter_selectivity) && length(input$modele_filter_selectivity) > 0) {
      df_filtered <- df_filtered[df_filtered$MODELE %in% input$modele_filter_selectivity, ]
    }
    if (!is.null(input$importer_filter_selectivity) && length(input$importer_filter_selectivity) > 0) {
      df_filtered <- df_filtered[df_filtered$CODE_IMPORTATEUR %in% input$importer_filter_selectivity, ]
    }
    if (!is.null(input$circuit_filter_selectivity) && length(input$circuit_filter_selectivity) > 0) {
      df_filtered <- df_filtered[df_filtered$CIRCUIT_PRECEDENT %in% input$circuit_filter_selectivity, ]
    }
    if (!is.null(input$nomenclature_filter_selectivity) && length(input$nomenclature_filter_selectivity) > 0) {
      df_filtered <- df_filtered[df_filtered$SH4 %in% input$nomenclature_filter_selectivity, ]
    }
    if (!is.null(input$origine_filter_selectivity) && length(input$origine_filter_selectivity) > 0) {
      df_filtered <- df_filtered[df_filtered$CODE_PAYS_ORIGINE %in% input$origine_filter_selectivity, ]
    }
    if (!is.null(input$Non_Conformity_filter_selectivity) && length(input$Non_Conformity_filter_selectivity) > 0) {
...
options(shiny.maxRequestSize = 800*1024^2) # Set max upload size to 300 MB
